...
const _sfc_main = {
  name: "UniClouddb",
  setup(props) {
    const dataListRef = props.getone ? common_vendor.shallowSsrRef(void 0, "xPuXDrLFoCd8LkFkA5cBtA==") : common_vendor.ssrRef([], "LNPRcRPOTQ2akGFgCU6oRQ==");
  ...
  methods: {
    ...
    add(value, {
      ...
      let db = common_vendor.St.database(this.spaceInfo);
      if (action) {
        db = db.action(action);
      }
    ...
    _execRemove(id, action, success, fail, complete, needConfirm, needLoading, loadingTitle) {
      ...
      const db = common_vendor.St.database(this.spaceInfo);
