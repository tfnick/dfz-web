...
const toDisplayString$1 = (val) => {
  return isString$1(val) ? val : val == null ? "" : isArray$2(val) || isObject$4(val) && (val.toString === objectToString$1 || !isFunction$1(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
const replacer = (_key, val) => {
  ...
  } else if (isObject$4(val) && !isArray$2(val) && !isPlainObject$1(val)) {
    return String(val);
  }
  return val;
};
...
const PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    if (setupState !== EMPTY_OBJ && setupState.__isScriptSetup && hasOwn$3(setupState, key)) {
    ...
    if (publicGetter) {
      ...
      if (data !== EMPTY_OBJ && (key[0] === "$" || key[0] === "_") && hasOwn$3(data, key)) {
  ...
  has({ _: { data, setupState, accessCache, ctx, appContext, propsOptions } }, key) {
    let normalizedProps;
    return !!accessCache[key] || data !== EMPTY_OBJ && hasOwn$3(data, key) || setupState !== EMPTY_OBJ && hasOwn$3(setupState, key) || (normalizedProps = propsOptions[0]) && hasOwn$3(normalizedProps, key) || hasOwn$3(ctx, key) || hasOwn$3(publicPropertiesMap, key) || hasOwn$3(appContext.config.globalProperties, key);
  },
...
const l = true, d = "mp-weixin", f = h('{\n    "address": [\n        "127.0.0.1",\n        "192.168.1.199"\n    ],\n    "debugPort": 53728,\n    "initialLaunchType": "local",\n    "servePort": 53730\n}\n'), p = h('[{"provider":"aliyun","spaceName":"unialiyun02","spaceId":"ca4b4f2a-3a78-4fb4-bc54-b6541df92422","clientSecret":"AjQBfd7x8dXVEARx3wXJSw==","endpoint":"https://api.bspapp.com"}]');
let m = "";
try {
  m = "__UNI__C0C2D81";
} catch (e2) {
